...
public abstract class BaseService<T, ID> {
  ...
  /**
   * Gets all.
   * better to do this on DB query (performance hit) check @getAllByQuery func
   *
  ...
  protected Mono<Page<T>> getAllByQuery(Pageable pageable) {
    ...
    return reactiveMongoTemplate.find(query, classType).collectList().flatMap(list ->
            reactiveMongoTemplate.count(new Query(), this.classType)
                    .subscribeOn(Schedulers.immediate()).map(count ->
                    new PageableResponse<T>(list, pageable.getPageNumber(), pageable.getPageSize(), pageable.getSort(), count)
            ));
